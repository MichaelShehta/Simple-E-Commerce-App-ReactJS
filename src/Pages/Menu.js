...
// // console.log(handelSortDSC);
  ...
  return (
    <div className="row mt-3">
      ...
      <button   value="ASC" onClick = {(e)=> handleSort(e) } type="button" class="btn btn-primary"><i class="fa-solid fa-sort-up"></i></button>
      <span style={{color:"white"}}>M</span>
      <button  value="DSC" onClick = {(e)=> handleSort(e) } type="button" class="btn btn-secondary"><i class="fa-solid fa-sort-down"></i></button>
      <span style={{color:"white"}}>M</span>
        <input onChange = {(e)=>handleSearch(e)} className="form-control me-2" type="search" placeholder="Search Here" aria-label="Search"/>
        ...
        <table className="table">
          ...
          <tbody>
            {search.length ? filteredSearch.map((p) => 
                 <tr key={p.id}>
                 <td>{p.name}</td>
                 <td>{p.price} $</td>
                 <td
